...
export default function Chatbottest() {
  ...
  }, [checkNotSure, liveAgent, agentName, agentInfoMsg, agentImage, messages]);
      ...
      try {
        fetchEventSource('/api/chat', {
          ...
          onmessage: async (event) => {
            if (event.data === '[DONE]') {
              // setCompletedMessage(event.data)
              // setCompletedMessage((prevCompletedMessage) => prevCompletedMessage + pending);
              if (pending) {
                // setCompletedMessage()
                // setCompletedMessage((prevCompletedMessage) => prevCompletedMessage + event.data);
                // setMessageState((state) => ({
              ...
              try {
                ...
                if (data.sourceDocs) {
                  ...
                  // setCompletedMessage((prevCompletedMessage) => prevCompletedMessage + data.data);
                  if (data.data.includes("Hmm")) {
                    setCheckNotSure(true);
                  }

                  // const translatedMessage = await handleTranslation(data.data, completedMessage);
  ...
  async function handleTranslation(response: string) {
    ...
    // const lastApiMessage = messageState.messages
    //               .filter(message => message.type === 'apiMessage')
    //               .pop();
    //             console.log("at last : ",lastApiMessage);
    // console.log("completedMessage : ", messages.filter)
    // if(!lastApiMessage){
      ...
      console.log('translate message',translatedMessage);
      setMessageState((state) => ({
        ...state,
        messages: [
          ...state.messages,
